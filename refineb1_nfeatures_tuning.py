...
b1_nfeatures = [50*i for i in range(1,21)]
single_dict = [[0.0 for j in b1_nfeatures] for i in gini_ratio]
...
_labels, report = detect_rare_B2(matrix_f, genes_f, labels_f, band_genes[3],
                                         A_global=graph, output_path=params.output_folder, conn_min=0.3, stab_min=0.5,
                                random_state=12277, n_pcs=20, k_knn=20, size_min_frac_parent=0.005, mix_alpha=0.7)

tab, gt_breakdown, ari, nmi = compare_clusters_filtered(params, _labels, labels, cells_f, cells)
print(f"ARI: {ari}, NMI: {nmi}")

for i, gr in enumerate(gini_ratio):
    for j, b1n in enumerate(b1_nfeatures):
        b1_genes, _ = select_genes(params,gene_stats, {"gini":gr, "palma":1.0-gr},b1n,3.5)
...
def generate_heatmap(data, fname):
    ...
    ax.set_yticklabels([f"{v:.2f}" for v in gini_ratio])
    ax.set_ylabel("Gini Ratio for B1 detection (Palma=1-Gini) (row)")

    ax.set_xticks(np.arange(len(b1_nfeatures)))
    ax.set_xticklabels([f"{v}" for v in b1_nfeatures], rotation=45, ha='right')
    ax.set_xlabel("B1 n features (column)")

    # colorbar
    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label("Cell type F1 or NMI/ARI")

    ax.set_title("Heatmap of F1/NMI/ARI vs (B1 refinement Gini(palma)/nfeatures)")
    fig.tight_layout()
...
folder = "b1refinement_nfeatures_94820"
for cell_type in cell_types:
