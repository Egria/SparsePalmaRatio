...
for i, gn in enumerate(gini_ngene):
    for j, pn in enumerate(fano_ngene):
        b5 = {"fano": 1.0}
...
def generate_heatmap(data, fname):
    ...
    ax.set_xticks(np.arange(len(fano_ngene)))
    ax.set_xticklabels([f"{v}" for v in fano_ngene], rotation=45, ha='right')
    ax.set_xlabel("Fano n genes")

    # colorbar
    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label("Cell type F1 or NMI/ARI")

    ax.set_title("Heatmap of F1/NMI/ARI vs (gini/fano n genes)")
    fig.tight_layout()
