...
for i, gr in enumerate(gini_ratio):
    for j, b2n in enumerate(b2_nfeatures):

        b5 = {"fano": 1.0}
        b4 = {"gini": 0.2, "fano": 0.8}
        b3 = {"gini": 1.0}
        b2 = {"gini": gr, "palma": 1.0 - gr}
        b1 = {"palma": 1.0}
...
def generate_heatmap(data, fname):
    ...
    ax.set_xticks(np.arange(len(b2_nfeatures)))
    ax.set_xticklabels([f"{v}" for v in b2_nfeatures], rotation=45, ha='right')
    ax.set_xlabel("B2 n features (column)")

    # colorbar
    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label("Cell type F1 or NMI/ARI")

    ax.set_title("Heatmap of F1/NMI/ARI vs (B2 refinement Gini(palma)/nfeatures)")
    fig.tight_layout()
